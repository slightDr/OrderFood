...
//    }
    ...
    public static void saveDefaultImgToPath(Context context, int id, String path) {
        Drawable defaultDrawable = ContextCompat.getDrawable(context, id);
        Bitmap defaltBitmap = ((BitmapDrawable) defaultDrawable).getBitmap();
        File file = new File(path);
        try (FileOutputStream fos = new FileOutputStream(file)) {
            defaltBitmap.compress(Bitmap.CompressFormat.PNG, 100, fos);
